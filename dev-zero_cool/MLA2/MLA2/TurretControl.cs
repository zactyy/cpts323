...
namespace TurretControl
{
    ...
    public class Turret
    {
        private bool DevicePresent; // API confirms there is a device present
        static private bool built = false;// API ensures single instance
        ...
        public Turret()//class constructor
        {
            if (!built)//ensure singleton status
            {
                built = true;
                this.UP = new byte[10];
                this.UP[1] = 2;
            ...
            else throw new SingletonException("Thats no Moon!!!! i.e thats a singleton dumbass!");
            //throw singleton exception if call violates singleton status
        }

        public void command_Stop()//sends stop command to turrent
        {
            this.SendUSBData(this.STOP);
        }

        public void command_Right(int degrees)//sends right command to turret
        {
            this.moveMissileLauncher(this.RIGHT, degrees);
        }

        public void command_Left(int degrees)//sends left command to turret
        {
            this.moveMissileLauncher(this.LEFT, degrees);
        }

        public void command_Up(int degrees)//sends up command to turrent
        {
            this.moveMissileLauncher(this.UP, degrees);
        }

        public void command_Down(int degrees)//sends down command to turret
        {
            this.moveMissileLauncher(this.DOWN, degrees);
        }

        public void command_Fire()//sends fire command to turret
        {
            this.moveMissileLauncher(this.FIRE, 5000);
        }

        public void command_switchLED(Boolean turnOn)//toggles led on turret
        {
            if (DevicePresent)
            {
                if (turnOn)
        ...
        public void command_reset()//resets turret position
        {
            if (DevicePresent)
            {
        ...
        private void moveMissileLauncher(byte[] Data, int interval)// sends usb commands to turret
        {
            if (DevicePresent)
            {
        ...
        private void SendUSBData(byte[] Data)//sends command data to turret
        {
